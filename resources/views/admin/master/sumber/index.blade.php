...
<div class="card">
    ...
    <div class="card-body row">
        <div class="col-md-12">
            @include('admin.master.sumber._table') 
        </div>
    </div>
</div>

@include('admin.master.sumber._modal')
@endsection
...
@section('scripts')
    <script type="text/javascript">
        ...
        function fetch_data(){
            ...
            $.ajax({
                url: "{{ url('admin/master/sumber/fetch') }}",
                dataType: 'json',
                success: function(res)
                {
                    let output = ''

                     $.each(res.sumbers, function(i, item){
                        output += `
                          <tr>
                            <td>${parseInt(i+1)}</td>
                            <td>
                              <div class="btn-group"><button class="btn btn-info btn-sm" id="handleEdit" data-id="${item.id}" data-name="${item.name}" title="edit ${item.name}"><i class="fa fa-edit cursor"></i></button>`
                                ...
                                output += `<button class="btn btn-danger btn-sm" id="handleDelete" data-id="${item.id}" url="{{ url('admin/master/sumber/destroy') }}" title="delete ${item.name}"><i class="fa fa-trash cursor"></i></button>`
                              }
                            output += `</td>
                              <td>${item.name}</td>
                          </tr>`
